...
class WebRoomServer extends events.EventEmitter
{
            ...
            case '/create_room':
                ...
                if (roomTypeVO) 
                {
                    this._wsServer.handleUpgrade(request, socket, head, (webSocket) => {
                        //TODO handle 'error' and close events on socket (remove all of the listeners which registered in this class)
    ...
    /**
     * @param {UnifiedSocket} unifiedServer 
     */
    _initUnifiedSocket(unifiedServer)
    {
        //TODO handle 'error' and close events on socket (remove all of the listeners which registered in this class)
        this._subscribeNextUnifiedSocketMessage(unifiedServer);
    }

    /**
     * @param {UnifiedSocket} socket 
     */
    _subscribeNextUnifiedSocketMessage(socket)
    {
        ...
        if (!isError) switch(messageObj.header)
        {
            case 'get_room_list':
                ...
                if (roomListType && this.isValidRoomType(roomListType))
                {
                    ...
                    this._subscribeNextUnifiedSocketMessage(socket);
                }
                else
                {
                    isError = true;
            ...
            case 'create_room':
                ...
                if (roomTypeVO)
                {
                    this.createAndJoinRoom(roomTypeVO, socket);
                }
                else
                {
                    isError = true;
    ...
    createAndJoinRoom(roomTypeVO, socket)
    {
        ...
        roomInstance.once('destroy', this._handleDestroyRoom.bind(this, webRoomVO));

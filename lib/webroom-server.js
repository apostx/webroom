...
class WebRoomServer extends events.EventEmitter
{
    ...
    createAndJoinRoom(roomTypeVO, socketEventTarget)
    {
        let roomId = null;
        do {
            const nextId = uuid.generateV4Id();
            
            if (!this._roomList.has(nextId))
            {
                roomId = nextId;
            }
        ...
        this._roomList.set(roomId, webRoomVO);
    ...
    _handleDestroyRoom(roomInstance)
    {
        ...
        for (const roomId of this._roomList.keys())
        {
            const webRoomVO = this._roomList.get(roomId);

            if (webRoomVO.roomInstance == roomInstance)
            {
    ...
    {
        return this._roomList.get(roomId);
    }
    ...
    getRoomList(roomType)
    {
        ...
        for (const roomId of this._roomList.keys())
        {
            ...
            if (webRoomVO.roomType == roomType && !webRoomVO.roomInstance.isHidden)
            {
                roomList.push({
                    id: roomId,
                    details: webRoomVO.roomInstance.details
                });
            }
        }

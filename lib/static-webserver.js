...
class StaticWebServer extends events.EventEmitter
{
    ...
    resolve(requestUrl)
    {
        ...
        const validRouteVO = this._routeList.find(this._validateRequest.bind(this, requestUrl));
     ...
     */
    _validateRequest(requestUrl, routeVO)
    {
        const filePathVO = path.parse(routeVO.filePath);
        
        return filePathVO.ext == "" ? this._validateRequestAsFolder(requestUrl, routeVO.requestUrl) : this._validateRequestAsFile(requestUrl, routeVO.requestUrl);
    }

    /**
     * @param {string} requestedUrl 
     * @param {string} supportedUrl 
     */
    _validateRequestAsFolder(requestedUrl, supportedUrl)
    {
        return !path.relative(supportedUrl, requestedUrl).startsWith("..");
    }
     ...
     * @param {string} supportedUrl 
     */
    _validateRequestAsFile(requestedUrl, supportedUrl)
    {

        return path.relative(supportedUrl, requestedUrl).length == 0;
    }
